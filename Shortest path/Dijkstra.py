...
def relax(u,v,d,w):
    ...
    Input:
        u: node, source
        v: node, destination
        d: dict, mapping each vertix to the best evaluated distance from
                source to the vertix.
...
if __name__ == "__main__":
    ...
    wieghts = [1,2,1,4,2,5,8,9,2]
    w ={}
    for i,tup in enumerate(tuples):
        g.add_edge(Edge(nodes[tup[0]],nodes[tup[1]]))
        w[(nodes[tup[0]],nodes[tup[1]])] = wieghts[i]
    source = g.nodes[0]
    print(f'source: {source}')
    distances = dijkstra(g,source,w)
