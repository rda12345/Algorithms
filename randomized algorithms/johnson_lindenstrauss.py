...
the first is defined by normally distributed random variables, while the latter is an Achlioptas scheme.
...
def jl_achlioptas(X: np.ndarray, m: int) -> np.ndarray:
    """
    Linear random projection, A, of a d-dimensional vector x, on to an m-dimensional space,
    where A_{ij} = +1/sqrt(m) with probability 1/6, -1/sqrt(m) with probability 1/6, and 0 with probability 2/3.
...
if __name__ == "__main__":
    ...
    print("---------- Achlioptas Variant --------")
    projected_X_achlioptas = jl_achlioptas(X, m)
